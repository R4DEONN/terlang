...
void Lexer::ScanInput(std::istream& input)
{
	...
	while (std::getline(input, line))
	{
		...
		for (int i = 0; i < line.size(); i++)
		{
			...
			else if (IsOperation(lexem[lexem.size() - 1]) && lexem[lexem.size() - 2] != 'e' && !IsOperation(line[i]) && m_state == LexerState::DEFAULT)
			{
				PushToken(lexem, row, pos);
				pos = i + 1;
			}
			...
			if (IsOperation(line[i]) && m_state == LexerState::DEFAULT)
			{
				if (i == 0 || line[i - 1] != 'e')
				{
					PushToken(lexem, row, pos);
